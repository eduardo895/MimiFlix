...
const RAPIDAPI_KEY = env.RAPIDAPI_KEY || env.TESTE_API_KEY || process.env.RAPIDAPI_KEY || process.env.TESTE_API_KEY || "";
const TESTE_API_HOST = env.TESTE_API_HOST || env.RAPIDAPI_HOST || process.env.TESTE_API_HOST || process.env.RAPIDAPI_HOST || "teste-api-xnxx.p.rapidapi.com";
const TESTE_API_BASE_URL = env.TESTE_API_BASE_URL || process.env.TESTE_API_BASE_URL || `https://${TESTE_API_HOST}`;
...
function ensureRapidApiKey(res) {
  if (RAPIDAPI_KEY) {
    return true;
  }

  sendJson(res, 503, {
    error: "Define RAPIDAPI_KEY ou TESTE_API_KEY no ficheiro .env para usar a API de videos."
  });
  return false;
}
...
function getRapidApiHeaders(contentLength) {
  const headers = {
    "x-rapidapi-host": TESTE_API_HOST,
...
async function fetchTesteApi(endpoint, { method = "GET", query = {}, body } = {}) {
  const url = new URL(endpoint, TESTE_API_BASE_URL);
  ...
  return requestJson(url, {
    method,
    headers: getRapidApiHeaders(Buffer.byteLength(bodyStr)),
    body: bodyStr
  });
}
...
async function handleRealCollections(requestUrl, res) {
  ...
  try {
    const sections = await Promise.all(
      REAL_COLLECTIONS.map(async (collection) => {
...
async function handleVideoSearch(req, res) {
  if (req.method !== "POST") {
    sendJson(res, 405, { error: "Metodo nao permitido." });
    return;
  }

  if (!ensureRapidApiKey(res)) {
    return;
  }
  ...
  try {
    ...
    const pages = parsePositiveInteger(payload?.pages, 1);

    if (!query) {
      sendJson(res, 400, { error: "q obrigatorio." });
      return;
    }

    const results = await fetchTesteApi("/api/search", {
      method: "POST",
      body: { q: query, pages }
    });

    sendJson(res, 200, Array.isArray(results) ? results : []);
  } catch (error) {
...
async function handleVideoDownload(req, res) {
  ...
  try {
    ...
    const results = await fetchTesteApi("/api/download", {
      method: "POST",
...
async function handleTrendingVideos(requestUrl, res) {
  if (!ensureRapidApiKey(res)) {
    return;
  }
  ...
  try {
    const results = await fetchTesteApi("/api/trending", {
      query: { page }
    });
...
async function handleLiveSports(requestUrl, res) {
  ...
  try {
    ...
    sendJson(res, 200, payload || { success: true, data: { matches: [] } });
  } catch (error) {
    sendJson(res, error.statusCode || 502, { error: error.message || "Nao foi possivel carregar os streams de desporto." });
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (requestUrl.pathname === "/api/search" && req.method === "POST") {
    await handleVideoSearch(req, res);
    return;
  }
  ...
  if (requestUrl.pathname === "/api/trending") {
    ...
    await handleTrendingVideos(requestUrl, res);
    return;
  }
  ...
  if (requestUrl.pathname === "/teste" || requestUrl.pathname === "/teste/") {
    sendFile(res, path.join(PUBLIC_DIR, "teste.html"));
    return;
  }
