...
let trailerPreviewCache = new Map();
...
async function fetchLiveSportsCatalog(filters = {}, options = {}) {
  ...
  const [catalog, health, watchfooty] = await Promise.all([
    fetchJson(`/api/live-sports${cacheKey ? `?${cacheKey}` : ""}`),
  ...
  const payload = { catalog, health, watchfooty };
...
function updateFavUI(movie) {
  ...
  if (selectedMovie && getMediaKey(selectedMovie) === getMediaKey(normalized)) {
    ...
    detailFavBtn.textContent = fav ? "♥ Favorito" : "♡ Favorito";
  }
}
...
function updateWatchlistUI(movie) {
  ...
  if (selectedMovie && getMediaKey(selectedMovie) === getMediaKey(normalized)) {
    ...
    detailWatchlistBtn.textContent = inWatchlist ? "✓ Na watchlist" : "＋ Watchlist";
  }
}
...
async function getTrailerPreviewKey(movie) {
  const normalized = normalizeMovie(movie);
  const numericId = Number(normalized?.id);
  if (!Number.isFinite(numericId)) return null;
  const cacheKey = `${getMediaType(normalized)}:${numericId}`;
  if (trailerPreviewCache.has(cacheKey)) {
    return trailerPreviewCache.get(cacheKey);
  }

  const language = encodeURIComponent(getLanguage());
  const endpoint = getMediaType(normalized) === "tv" ? "tv" : "movie";
  const response = await fetchJson(`/api/${endpoint}/${numericId}/trailer?language=${language}`).catch(() => ({ key: null }));
  trailerPreviewCache.set(cacheKey, response?.key || null);
  return trailerPreviewCache.get(cacheKey);
}
...
function refreshBadgeExperience(options = {}) {
  ...
  if (options.refreshRoute && ["/perfil", "/conquistas"].includes(getCurrentPath())) {
    handleRoute();
  }
...
async function viewDesporto() {
  ...
  const renderSports = async (options = {}) => {
    ...
    try {
      const { catalog, health, watchfooty } = await fetchLiveSportsCatalog(state, { force: options.force });
      const data = catalog?.data || {};
      ...
      const watchFootyMatches = (Array.isArray(watchfooty?.data?.matches) ? watchfooty.data.matches : [])
        ...
        .filter((match) => match.title);
      const matches = [...vipMatches, ...watchFootyMatches]
        .sort((left, right) => {
...
async function viewFavoritos() {
  ...
  if (!favsCache.length) {
    const empty = document.createElement("div");
    empty.className = "empty-state";
    empty.innerHTML = `<div class="empty-icon">♡</div><p>Ainda não tens filmes favoritos.</p>`;
    appContent.appendChild(empty);
    return;
  }
...
async function viewWatchlist() {
  ...
  if (!watchlistCache.length) {
    const empty = document.createElement("div");
    empty.className = "empty-state";
    empty.innerHTML = `<div class="empty-icon">＋</div><p>A tua watchlist ainda está vazia. Usa o botão Watchlist nos detalhes de qualquer filme.</p>`;
    appContent.appendChild(empty);
    return;
  }
...
async function viewHistorico() {
  ...
  if (!events.length) {
    const empty = document.createElement("div");
    empty.className = "empty-state";
    empty.innerHTML = `<div class="empty-icon">⏱</div><p>A tua timeline vai ganhando vida quando vês, favoritas e avalias filmes.</p>`;
    appContent.appendChild(empty);
    return;
  }
...
async function viewAmigos() {
  appContent.innerHTML = "";
  appContent.appendChild(pageHeader("Amigos e partilha"));

  const socialRecommendations = buildSocialRecommendations();
  const shell = document.createElement("div");
  shell.className = "lists-shell";
  shell.innerHTML = `
    <section class="lists-create-card">
      <h2 class="lists-title">Importar código de amigo</h2>
      <div class="lists-create-row">
        <input id="friendShareCode" class="lists-input" type="text" placeholder="Cola aqui o código que te enviaram">
        <button id="importFriendBtn" class="accent-btn" type="button">Importar</button>
      </div>
      <div class="profile-actions" style="margin-top:14px">
        <button id="shareOwnProfileBtn" class="secondary-btn" type="button">Copiar o meu perfil</button>
      </div>
    </section>
    <section class="profile-panel">
      <div class="profile-panel-head">
        <h3 class="profile-panel-title">Recomendações sociais</h3>
        <span class="profile-panel-tag">${followedProfilesCache.length ? "Com base nos teus amigos" : "Importa perfis para desbloquear"}</span>
      </div>
      <div id="socialRecommendations" class="genre-grid"></div>
    </section>
    <section id="friendsContainer" class="lists-grid"></section>
  `;
  appContent.appendChild(shell);

  const friendsContainer = shell.querySelector("#friendsContainer");
  const recommendationsGrid = shell.querySelector("#socialRecommendations");

  if (socialRecommendations.length) {
    renderMovieCards(recommendationsGrid, socialRecommendations);
  } else {
    recommendationsGrid.innerHTML = `<div class="empty-state" style="grid-column:1/-1">Quando importares amigos, vais começar a ver filmes populares entre os perfis que segues.</div>`;
  }
  ...
  function renderFriends() {
    ...
    if (!followedProfilesCache.length) {
      friendsContainer.innerHTML = `<div class="empty-state">Ainda não segues ninguém. Pede um código de partilha a um amigo e importa-o aqui.</div>`;
      return;
    }
    ...
    followedProfilesCache.forEach((profile) => {
        ...
        <div class="profile-panel-head">
          <div class="profile-hero-main">
            ...
            <div class="profile-copy">
              <h3 class="profile-panel-title">${escapeHtml(profile.displayName)}</h3>
              <p class="profile-email">Partilhado ${escapeHtml(formatShortDateTime(profile.exportedAt))}</p>
            </div>
          </div>
          <div class="profile-actions">
            <button class="secondary-btn" type="button" data-action="remove">Remover</button>
          </div>
        </div>
        <div class="profile-summary-list">
          <div class="profile-summary-item"><span class="profile-summary-bullet"></span><p>${profile.favorites.length} favorito${profile.favorites.length === 1 ? "" : "s"} partilhado${profile.favorites.length === 1 ? "" : "s"}.</p></div>
          <div class="profile-summary-item"><span class="profile-summary-bullet"></span><p>${profile.lists.length} lista${profile.lists.length === 1 ? "" : "s"} disponível${profile.lists.length === 1 ? "" : "eis"} para explorar.</p></div>
        </div>
        <div class="profile-review-list" style="margin-top:18px">
          <section>
            <div class="profile-panel-head">
              <h4 class="profile-review-title">Favoritos</h4>
            </div>
            <div class="genre-grid friend-favorites-grid"></div>
          </section>
          ${profile.lists.length ? profile.lists.map((list) => `
            <section>
              <div class="profile-panel-head">
                <h4 class="profile-review-title">${escapeHtml(list.name)}</h4>
              </div>
              <div class="genre-grid friend-list-grid" data-list-id="${list.id}"></div>
            </section>
          `).join("") : ""}
        </div>
      `;
      ...
      card.querySelector('[data-action="remove"]').addEventListener("click", () => {
        removeFollowedProfile(profile.shareCode);
        renderFriends();
        handleRoute();
      });

      renderMovieCards(card.querySelector(".friend-favorites-grid"), profile.favorites.slice(0, 8));
      profile.lists.forEach((list) => {
        const grid = card.querySelector(`[data-list-id="${list.id}"]`);
        if (grid) {
          renderMovieCards(grid, list.movies.slice(0, 8));
        }
      });
  ...
  shell.querySelector("#importFriendBtn").addEventListener("click", async () => {
    const input = shell.querySelector("#friendShareCode");
    try {
      await importFollowedProfile(input.value);
      input.value = "";
      handleRoute();
    } catch (error) {
      window.alert(error.message || "Nao foi possivel importar este código.");
    }
  });

  shell.querySelector("#shareOwnProfileBtn").addEventListener("click", async () => {
    try {
      await shareProfileSnapshot();
      window.alert("Código do teu perfil copiado. Envia-o a um amigo.");
    } catch {
      window.alert("Nao foi possivel copiar o teu perfil.");
    }
  });
...
async function viewColecoes() {
  ...
  const [sections, profileSections, realSections] = await Promise.all([
    ensureSmartCollections().catch(() => []),
    ensureProfileCollectionsData().catch(() => []),
    ensureRealCollections().catch(() => [])
  ]);
  if (!sections.length && !profileSections.length && !realSections.length) {
    appContent.innerHTML += errorState("Nao foi possivel carregar as coleções automáticas.");
    return;
  }
  ...
  if (decoratedRealSections.length) {
    ...
    decoratedRealSections.forEach((section) => {
      const card = document.createElement("article");
      card.className = "profile-stat-card collection-stat-card";
      card.innerHTML = `
        <span class="profile-stat-label">${escapeHtml(section.title)}</span>
        <strong class="profile-stat-value">${escapeHtml(section.stats?.map((item) => item.value).join(" • ") || "Coleção")}</strong>
        <p class="profile-stat-note">${escapeHtml(section.context || section.description || "Coleção viva com ordem recomendada e progresso.")}</p>
      `;
      overview.appendChild(card);
    });
    appContent.appendChild(overview);
  }

  if (profileSections.length) {
    renderCatalogRows(profileSections, appContent);
  }
  if (decoratedRealSections.length) {
    renderCatalogRows(decoratedRealSections, appContent);
  }
  if (sections.length) {
    renderCatalogRows(sections, appContent);
  }
}
...
async function viewPerfil() {
  ...
  shell.innerHTML = `
      ...
      <div class="profile-hero-main">
        ...
        <div class="profile-copy">
          ...
          <div class="profile-meta-line">
            ...
            <span class="chip">Idioma atual ${escapeHtml(profile.language)}</span>
          </div>
        </div>
      </div>
      <div class="profile-actions">
        <button class="secondary-btn" type="button" data-profile-action="favoritos">Ver favoritos</button>
        <button class="secondary-btn" type="button" data-profile-action="watchlist">Abrir watchlist</button>
        <button class="secondary-btn" type="button" data-profile-action="historico">Ver histórico</button>
        <button class="secondary-btn" type="button" data-profile-action="amigos">Amigos</button>
        <button class="secondary-btn" type="button" data-profile-action="resumo">Resumo anual</button>
        <button class="secondary-btn" type="button" data-profile-action="mood">Mood picker</button>
        <button class="secondary-btn" type="button" data-profile-action="estreias">Calendário pessoal</button>
        <button class="secondary-btn" type="button" data-profile-action="home-editor">Editar início</button>
        <button class="secondary-btn" type="button" data-profile-action="perfis">Perfis</button>
        <button class="secondary-btn" type="button" data-profile-action="conquistas">Conquistas</button>
        <button class="secondary-btn" type="button" data-profile-action="share">Partilhar perfil</button>
        <button class="secondary-btn" type="button" data-profile-action="filmes">Explorar filmes</button>
    ...
    <section class="profile-panels">
      ...
      <article class="profile-panel">
        <div class="profile-panel-head">
          <h3 class="profile-panel-title">Resumo rapido</h3>
          <span class="profile-panel-tag">O teu momento no MimiFlix</span>
        </div>
        <div class="profile-summary-list">
          ${profile.highlights.map((item) => `
            <div class="profile-summary-item">
              <span class="profile-summary-bullet"></span>
              <p>${escapeHtml(item)}</p>
            </div>
          `).join("")}
        </div>
      ...
      <article class="profile-panel">
        <div class="profile-panel-head">
          <h3 class="profile-panel-title">Conquistas</h3>
          <span class="profile-panel-tag">${profile.badgeSummary.earnedCount}/${BADGE_DEFINITIONS.length} desbloqueadas</span>
        </div>
        ${renderBadgeOverview(profile.badgeSummary, { limit: 4 })}
      </article>
      <article class="profile-panel">
        <div class="profile-panel-head">
        ...
        <div class="profile-avatar-editor">
          ...
          <div class="profile-avatar-upload-copy">
            ...
            <p id="profileAvatarStatus" class="profile-avatar-status">${profile.avatarImage ? "Foto atual carregada." : "Sem foto carregada. Vais usar o avatar automatico."}</p>
          </div>
        </div>
        <div class="profile-actions"><button class="accent-btn" type="button" id="saveProfilePrefsBtn">Guardar perfil</button></div>
      </article>
      <article class="profile-panel">
        <div class="profile-panel-head">
  ...
  shell.querySelectorAll("[data-profile-action]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      else if (action === "amigos") navigateTo("/amigos");
      else if (action === "resumo") navigateTo("/resumo-anual");
      else if (action === "mood") navigateTo("/mood");
      else if (action === "estreias") navigateTo("/estreias");
      else if (action === "home-editor") navigateTo("/inicio-editor");
      else if (action === "perfis") navigateTo("/perfis");
      else if (action === "conquistas") navigateTo("/conquistas");
      else if (action === "share") await shareProfileSnapshot().then(() => window.alert("Código do teu perfil copiado. Partilha-o na página Amigos."));
      else if (action === "filmes") navigateTo("/filmes");
  ...
  syncAvatarDraftUi(profile.avatarImage ? "Foto atual carregada." : "Sem foto carregada. Vais usar o avatar automatico.");
  ...
  if (profileAvatarFileInput) {
    profileAvatarFileInput.addEventListener("change", async () => {
      ...
      try {
        ...
        syncAvatarDraftUi(err?.message || "Nao foi possivel atualizar a foto.");
      } finally {
  ...
  if (removeProfileAvatarBtn) {
    removeProfileAvatarBtn.addEventListener("click", () => {
      if (!draftAvatarImage) {
        syncAvatarDraftUi("Ja estas a usar o avatar automatico.");
        return;
      }
      draftAvatarImage = "";
...
async function viewConquistas() {
  appContent.innerHTML = "";
  appContent.appendChild(pageHeader("Conquistas"));
  ...
  shell.innerHTML = `
    <section class="profile-hero-card badge-hero-card">
      <div class="profile-copy">
        <span class="eyebrow">Sistema de badges</span>
        <h2 class="profile-name">O teu progresso no MimiFlix</h2>
        <p class="profile-email">${summary.earnedCount} de ${BADGE_DEFINITIONS.length} conquistas desbloqueadas.</p>
        <div class="profile-meta-line">
          <span class="chip">Listas partilhadas ${badgeStats.listShareCount}</span>
          <span class="chip">Perfil partilhado ${badgeStats.profileShareCount}</span>
        </div>
      </div>
      <div class="profile-summary-list">
        <div class="profile-summary-item">
          <span class="profile-summary-bullet"></span>
          <p>${summary.nextBadge ? `Próxima badge: "${summary.nextBadge.title}" com ${Math.min(summary.nextBadge.current, summary.nextBadge.target)}/${summary.nextBadge.target}.` : "Já tens todas as badges atuais desbloqueadas."}</p>
        </div>
        <div class="profile-summary-item">
          <span class="profile-summary-bullet"></span>
          <p>${currentUser ? "As conquistas ficam sincronizadas com a tua conta." : "Sem sessão iniciada, o progresso fica guardado neste dispositivo."}</p>
        </div>
      </div>
    </section>
    <section class="profile-stats-grid">
      <article class="profile-stat-card">
        <span class="profile-stat-label">Desbloqueadas</span>
        <strong class="profile-stat-value">${summary.earnedCount}</strong>
        <p class="profile-stat-note">Badges já garantidas na tua conta.</p>
      </article>
      <article class="profile-stat-card">
        <span class="profile-stat-label">Por desbloquear</span>
        <strong class="profile-stat-value">${BADGE_DEFINITIONS.length - summary.earnedCount}</strong>
        <p class="profile-stat-note">${summary.nextBadge ? `A próxima é ${summary.nextBadge.title}.` : "Já concluíste a coleção atual."}</p>
      </article>
      <article class="profile-stat-card">
        <span class="profile-stat-label">Partilhas</span>
        <strong class="profile-stat-value">${badgeStats.profileShareCount + badgeStats.listShareCount}</strong>
        <p class="profile-stat-note">Perfis e listas que já enviaste para amigos.</p>
      </article>
    </section>
    <section class="profile-panel">
      <div class="profile-panel-head">
        <h3 class="profile-panel-title">Todas as badges</h3>
        <span class="profile-panel-tag">Desbloqueadas e em progresso</span>
      </div>
      ${renderBadgeCards(summary.badges)}
    </section>
  `;
...
function renderHero(movie) {
  ...
  heroPlayBtn.textContent = getMediaType(movie) === "tv" ? "▶ Ver episódio" : "▶ Ver filme";
}
...
function hideMoviePreview(card) {
  const preview = card.querySelector(".movie-preview-card");
  if (!preview) return;
...
async function showMoviePreview(card, movie) {
  const preview = card.querySelector(".movie-preview-card");
  if (!preview) return;
  card.classList.add("is-preview-expanded");
  ...
  preview.innerHTML = `
    <div class="movie-preview-media">
      <img src="${backdrop}" alt="${escapeHtml(movie.title)}">
    </div>
    <div class="movie-preview-copy">
      <strong>${escapeHtml(movie.title)}</strong>
      <div class="movie-preview-meta">
        <span>${escapeHtml(year)}</span>
        ${rating ? `<span>★ ${rating}</span>` : ""}
      </div>
      <p>${escapeHtml((movie.overview || "Pré-visualização rápida disponível neste cartão.").slice(0, 120))}</p>
      <div class="movie-preview-actions">
        <button class="secondary-btn" type="button" data-preview-open-details="${movie.id}">Detalhes</button>
      </div>
    </div>
  `;

  preview.querySelector("[data-preview-open-details]")?.addEventListener("click", (event) => {
    event.stopPropagation();
    openDetails(movie);
  });

  const trailerKey = movie.trailerKey || await getTrailerPreviewKey(movie);
  if (!card.matches(":hover") && !card.matches(":focus-within")) {
    hideMoviePreview(card);
    return;
  }

  if (trailerKey) {
    preview.innerHTML = `
      <div class="movie-preview-media is-trailer">
        <iframe src="https://www.youtube.com/embed/${trailerKey}?autoplay=1&mute=1&controls=0&rel=0&playsinline=1" title="Trailer de ${escapeHtml(movie.title)}" allow="autoplay; encrypted-media; picture-in-picture" loading="lazy"></iframe>
      </div>
      <div class="movie-preview-copy">
        <strong>${escapeHtml(movie.title)}</strong>
        <div class="movie-preview-meta">
          <span>${escapeHtml(year)}</span>
          ${rating ? `<span>★ ${rating}</span>` : ""}
          <span>Trailer autoplay</span>
        </div>
        <p>${escapeHtml((movie.overview || "Pré-visualização rápida disponível neste cartão.").slice(0, 120))}</p>
        <div class="movie-preview-actions">
          <button class="secondary-btn" type="button" data-preview-open-trailer="${trailerKey}">Abrir trailer</button>
          <button class="secondary-btn" type="button" data-preview-open-details="${movie.id}">Detalhes</button>
        </div>
      </div>
    `;

    preview.querySelector("[data-preview-open-trailer]")?.addEventListener("click", (event) => {
      event.stopPropagation();
      openTrailer(trailerKey, movie.title);
    });
    preview.querySelector("[data-preview-open-details]")?.addEventListener("click", (event) => {
      event.stopPropagation();
      openDetails(movie);
...
function attachMoviePreviewInteractions(card, movie) {
  let hoverTimer = null;

  const startPreview = () => {
    clearTimeout(hoverTimer);
    hoverTimer = setTimeout(() => {
      void showMoviePreview(card, movie);
    }, 420);
  };
...
function renderMovieCards(container, movies, options = {}) {
  movies.forEach((movie, i) => {
    ...
    const metaLabel = escapeHtml(movie.social_context || (year || "—"));
    ...
    card.innerHTML = `
      ...
      <div class="movie-card-info">
        ...
        <div class="movie-card-meta">${metaLabel}</div>
      </div>
      <div class="movie-preview-card"></div>
    `;
    ...
    attachMoviePreviewInteractions(card, movie);
...
function renderSeriesPanel(series) {
  ...
  detailSeriesPanel.innerHTML = `
    <section class="detail-series-card">
      <div class="profile-panel-head">
        <div>
          <h3 class="profile-panel-title">Episódios</h3>
          <p class="detail-review-subtitle">Escolhe a temporada e o episódio antes de abrires o player.</p>
        </div>
        <span class="chip">${series.number_of_seasons || seasons.length || 1} temporadas</span>
      </div>
      <div class="detail-series-spotlight">
        <div class="detail-series-spotlight-copy">
          <div class="detail-series-spotlight-meta">
            <span class="chip chip-accent">${escapeHtml(formatEpisodeLabel(selectedSeason, selectedEpisode) || "Episódio selecionado")}</span>
            ${resumeTarget?.hasProgress ? `<span class="chip">${resumeTarget.progressPercent}% visto</span>` : ""}
            ${nextTarget ? `<span class="chip">Seguinte ${escapeHtml(formatEpisodeLabel(nextTarget.season, nextTarget.episode))}</span>` : ""}
          </div>
          <strong>${escapeHtml(selectedEpisodeMeta?.name || `Episódio ${selectedEpisode}`)}</strong>
          <p class="detail-review-subtitle">${escapeHtml(
            selectedEpisodeMeta?.overview
            || (resumeTarget?.hasProgress
              ? `Retoma de onde paraste e avança logo para o episódio seguinte sem saíres da série.`
              : `Escolhe um episódio da lista ou usa os atalhos rápidos para continuar a maratona.`)
          )}</p>
        </div>
        <div class="detail-series-actions">
          <button class="accent-btn" type="button" data-series-action="watch-selected">▶ Ver ${escapeHtml(formatEpisodeLabel(selectedSeason, selectedEpisode) || "episódio")}</button>
          ${showResumeButton ? `<button class="secondary-btn" type="button" data-series-action="resume">${resumeTarget.hasProgress ? `Retomar ${escapeHtml(formatEpisodeLabel(resumeTarget.season, resumeTarget.episode))}` : `Ir para ${escapeHtml(formatEpisodeLabel(resumeTarget.season, resumeTarget.episode))}`}</button>` : ""}
          ${nextTarget ? `<button class="secondary-btn" type="button" data-series-action="next">Seguinte ${escapeHtml(formatEpisodeLabel(nextTarget.season, nextTarget.episode))}</button>` : ""}
        </div>
      </div>
      <div class="player-settings-grid">
        <label class="filter-field">
          ...
          <select id="detailSeasonSelect">
            ${seasons.map((season) => `<option value="${season.season_number}"${season.season_number === selectedSeason ? " selected" : ""}>${escapeHtml(season.name || `Temporada ${season.season_number}`)}</option>`).join("")}
          </select>
        </label>
      </div>
      <div class="detail-episode-grid">
        ${episodes.length ? episodes.map((episode) => `
          <button class="detail-episode-chip${episode.episode_number === selectedEpisode ? " is-active" : ""}" type="button" data-detail-episode="${episode.episode_number}">
            <div class="detail-episode-chip-thumb">
              <img
                class="detail-episode-chip-image"
                src="${episode.still_path ? `${POSTER_BASE_URL}${episode.still_path}` : PLACEHOLDER_STILL}"
                alt="${escapeHtml(episode.name || `Episódio ${episode.episode_number}`)}"
                loading="lazy"
              >
              <span class="detail-episode-num">${episode.episode_number}</span>
            </div>
            <div class="detail-episode-chip-body">
              <div class="detail-episode-chip-header">
                <strong>${escapeHtml(episode.name || `Episódio ${episode.episode_number}`)}</strong>
                ${episode.runtime ? `<span class="detail-episode-duration">${episode.runtime} min</span>` : ""}
              </div>
              ${episode.overview ? `<p class="detail-episode-overview">${escapeHtml(episode.overview)}</p>` : ""}
            </div>
          </button>
  ...
  detailSeriesPanel.querySelector("#detailSeasonSelect")?.addEventListener("change", (event) => {
  ...
  detailSeriesPanel.querySelectorAll("[data-detail-episode]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      detailSeriesPanel.querySelectorAll(".detail-episode-chip").forEach((b) => b.classList.remove("is-active"));
      button.classList.add("is-active");
...
async function renderDetailContent(mediaRef) {
  ...
  detailFavBtn.textContent = "♡ Favorito";
  detailWatchlistBtn.classList.remove("is-fav");
  detailWatchlistBtn.textContent = "＋ Watchlist";
  detailPlayBtn.textContent = isTv ? "▶ Ver episódio" : "▶ Ver filme";
...
function renderPlayerProviderTabs(movie) {
  ...
  if (!movie) {
    playerProviderTabs.classList.add("hidden");
    playerProviderTabs.innerHTML = "";
    return;
  }

  const activeProvider = String(activePlaybackSession?.provider || getCurrentPlayerProvider()).toLowerCase();
  playerProviderTabs.classList.remove("hidden");
  playerProviderTabs.innerHTML = PLAYER_EMBED_PROVIDERS.map((provider) => `
    <button
  ...
  playerProviderTabs.querySelectorAll("[data-player-provider]").forEach((button) => {
    button.addEventListener("click", () => {
...
function buildProfileSummary() {
  ...
  return {
    ...
    stats: [
      ...
      {
        label: "Conquistas",
        value: `${badgeSummary.earnedCount}/${BADGE_DEFINITIONS.length}`,
        note: badgeSummary.earnedCount ? "Badges já desbloqueadas" : "Explora a app para começares a ganhar badges"
      },
      {
        label: "Tua nota",
      ...
      {
        label: "Amigos",
        value: String(followedProfilesCache.length),
        note: followedProfilesCache.length ? "Perfis importados para recomendações sociais" : "Importa perfis para ativar a vertente social"
      },
      {
        label: "Periodo",
...
function renderBadgeOverview(summary, options = {}) {
  ...
  return `
    <div class="badge-overview">
      ...
      <div class="profile-actions">
        <button class="secondary-btn" type="button" data-profile-action="conquistas">Abrir conquistas</button>
      </div>
    </div>
  `;
}
...
function buildMetaHtml(movie, includeGenres = false) {
  const chips = [];
  if (getMediaType(movie) === "tv") chips.push('<span class="chip">Série</span>');
  if (movie.release_date) chips.push(`<span class="chip">${escapeHtml(movie.release_date.slice(0, 4))}</span>`);
  if (typeof movie.vote_average === "number" && movie.vote_average > 0)
    chips.push(`<span class="chip chip-accent">&#9733; ${movie.vote_average.toFixed(1)}</span>`);
  if (movie.original_language)
    chips.push(`<span class="chip">${escapeHtml(String(movie.original_language).toUpperCase())}</span>`);
  if (getMediaType(movie) === "tv" && movie.number_of_seasons) {
...
function renderSpinner() { return `<div class="loading-spinner"></div>`; }
